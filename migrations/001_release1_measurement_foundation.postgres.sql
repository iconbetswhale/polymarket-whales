...
CREATE INDEX IF NOT EXISTS idx_candidate_ledger_decision_seen ON candidate_ledger(current_decision, last_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_candidate_ledger_event ON candidate_ledger(canonical_event_id, canonical_market_id, canonical_outcome_id);
CREATE INDEX IF NOT EXISTS idx_candidate_ledger_correlation ON candidate_ledger(correlation_id, last_seen_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_candidate_decisions_candidate_time ON candidate_decisions(candidate_id, decided_at DESC);
CREATE INDEX IF NOT EXISTS idx_candidate_decisions_outcome_time ON candidate_decisions(decision, decided_at DESC);

CREATE TABLE IF NOT EXISTS candidate_monitoring (
    candidate_id TEXT PRIMARY KEY REFERENCES candidate_ledger(candidate_id),
    ...
    exchange_closing_price DOUBLE PRECISION,
    composite_closing_probability DOUBLE PRECISION,
    exchange_probability_point_clv DOUBLE PRECISION,
    exchange_stake_return_clv DOUBLE PRECISION,
    composite_probability_point_clv DOUBLE PRECISION,
    composite_stake_return_clv DOUBLE PRECISION,
    execution_loss DOUBLE PRECISION,
    fee_adjusted_clv DOUBLE PRECISION,
    closing_timestamp TEXT,
    result TEXT,
    hypothetical_stake DOUBLE PRECISION NOT NULL DEFAULT 100.0,
    hypothetical_profit_loss DOUBLE PRECISION,
    maximum_favorable_movement DOUBLE PRECISION,
    maximum_adverse_movement DOUBLE PRECISION,
    pass_reason_justified BOOLEAN,
    missing_reason TEXT,
...
CREATE INDEX IF NOT EXISTS idx_dual_clv_tracker_time ON dual_clv_measurements(tracker_type, user_id, closing_timestamp DESC);

CREATE TABLE IF NOT EXISTS composite_price_snapshots (
    snapshot_id TEXT PRIMARY KEY,
    candidate_id TEXT REFERENCES candidate_ledger(candidate_id),
    correlation_id TEXT NOT NULL,
    quote_timestamp TEXT NOT NULL,
    composite_fair_probability DOUBLE PRECISION,
    source_count INTEGER NOT NULL DEFAULT 0,
    source_dispersion DOUBLE PRECISION,
...
CREATE INDEX IF NOT EXISTS idx_composite_snapshots_candidate_time ON composite_price_snapshots(candidate_id, quote_timestamp DESC);

CREATE TABLE IF NOT EXISTS composite_source_contributions (
    snapshot_id TEXT NOT NULL REFERENCES composite_price_snapshots(snapshot_id),
    provider TEXT NOT NULL,
    ...
    raw_implied_probability DOUBLE PRECISION,
