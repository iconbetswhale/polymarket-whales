...
`ODDSENGINE_API_KEY=`
Server-only OddsEngine key used as the sole live sportsbook-aggregation feed for Positive EV, Arbitrage, Middles, Low Hold, Odds Screen, Fantasy Optimizer, Sharp Money, model fair pricing, and sportsbook line shopping. Requests authenticate with `X-API-Key`; the key is never sent to the browser or included in diagnostics. Direct NoVIG NBX, Polymarket, ProphetX, 4CX, and Kalshi integrations remain independent exchange sources. The standard event and odds endpoints feed IconLabs' existing calculators and models, so provider changes do not alter their math. With an Advanced or custom Advanced-enabled key, Sharp Money reads `/v1/orderbook/top?sort=whale&selected_books=novig,prophetx` for both exchanges' two-sided depth, liquidity, retail peers, edge, and whale-volume fields; separate direct ProphetX credentials are optional for read-only analysis but remain required for direct account execution.

`ODDSENGINE_API_BASE_URL=https://api.oddsengine.dev/v1`
OddsEngine API base URL. A durable shared raw-event snapshot serves every compatible live page and scheduled LabTracker job, so separate serverless instances reuse the same provider response instead of spending the request budget independently. Browser requests are targeted to the selected league and market, and the full Odds Screen is loaded only on user intent. Opportunity scanners stay capped at 5 upcoming events per league and 20 total events to remain inside the provider request budget; the Odds Screen can consume up to 50 events for a selected full slate. The adapter records rate-limit headers and returns already-fetched real events as a partial snapshot if a scan reaches the limit. It does not substitute SportsGameOdds, The Odds API, demo odds, or generated prices when OddsEngine is unavailable.
...
`PROPHETX_API_BASE_URL=https://api.sandbox.prophetx.dev/partner`
...
`PROPHETX_TRADE_URL=https://sandbox.prophetx.dev/`
