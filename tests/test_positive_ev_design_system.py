...
def test_positive_ev_opts_into_v2_without_legacy_layers() -> None:
    canonical_pages = "['trades', 'positive-ev', 'sharp-money', 'odds-screen', 'dfs', 'tracker', 'lab-tracker', 'shadow-test', 'live-positions', 'wallets', 'position-history', 'edge-map', 'intelligence']"
    ...
    for stylesheet in ("app-premium.css", "sidebar-shell.css"):
        assert (
            f"page not in ['home', 'trades', 'positive-ev', 'sharp-money', 'odds-screen', 'dfs', 'tracker', 'lab-tracker', 'shadow-test', 'live-positions', 'wallets', 'position-history', 'edge-map', 'intelligence'] %}}<link rel=\"stylesheet\" "
            f"href=\"{{{{ url_for('static', filename='{stylesheet}'"
        ) in BASE
