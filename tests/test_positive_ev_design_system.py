...
def test_positive_ev_opts_into_v2_without_legacy_layers() -> None:
    canonical_pages = "['trades', 'positive-ev', 'sharp-money', 'odds-screen', 'dfs', 'tracker']"
    ...
    for stylesheet in ("app-premium.css", "sidebar-shell.css"):
        assert (
            f"page not in ['home', 'trades', 'positive-ev', 'sharp-money', 'odds-screen', 'dfs', 'tracker'] %}}<link rel=\"stylesheet\" "
            f"href=\"{{{{ url_for('static', filename='{stylesheet}'"
        ) in BASE
