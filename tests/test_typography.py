...
def test_canonical_pages_opt_into_the_v2_foundation_last():
    ...
    assert 'data-design-system="{% if page in [\'trades\', \'positive-ev\', \'sharp-money\', \'odds-screen\', \'dfs\', \'tracker\', \'lab-tracker\', \'shadow-test\', \'live-positions\', \'wallets\', \'position-history\', \'edge-map\', \'intelligence\'] %}v2' in template
...
def test_canonical_pages_do_not_reload_legacy_override_layers():
    ...
    for stylesheet in (
        ...
        excluded_for_canonical_pages = (
            f"page not in ['trades', 'positive-ev', 'sharp-money', 'odds-screen', 'dfs', 'tracker', 'lab-tracker', 'shadow-test', 'live-positions', 'wallets', 'position-history', 'edge-map', 'intelligence'] %}}<link rel=\"stylesheet\" href=\"{{{{ url_for('static', filename='{stylesheet}'"
            in template
        )
        excluded_for_home_and_canonical_pages = (
            f"page not in ['home', 'trades', 'positive-ev', 'sharp-money', 'odds-screen', 'dfs', 'tracker', 'lab-tracker', 'shadow-test', 'live-positions', 'wallets', 'position-history', 'edge-map', 'intelligence'] %}}<link rel=\"stylesheet\" href=\"{{{{ url_for('static', filename='{stylesheet}'"
            in template
        )
